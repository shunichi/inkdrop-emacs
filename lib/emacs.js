...
var _killRing = _interopRequireDefault(require("./killRing"));
...
module.exports = function (CodeMirror) {
  ...
  var killRing = new _killRing.default();

  function addToRing(str) {
    killRing.push(str);

    _electron.clipboard.writeText(str);
  }

  function growRingTop(str) {
    killRing.grow(str);

    _electron.clipboard.writeText(killRing.top());
  }
  ...
  function rememberYank(cm, start, end) {
    if (lastYank == null) {
  ...
  function forgetYank() {
    if (lastYank) {
      lastYank.cm.off('cursorActivity', onCursorActivity);
      lastYank = null;
    }
  }
  ...
  function yank(cm) {
    ...
    rememberYank(cm, start, cm.getCursor());
  }

  function yankPop(cm) {
    if (lastYank && lastYank.cm == cm && cm.isClean(lastYank.gen)) {
      const text = popFromRing();
      const start = lastYank.start;
      cm.replaceRange(text, lastYank.start, lastYank.end, "paste");
      rememberYank(cm, start, cm.getCursor());
    } else {
      console.log('Previous command was not a yank');
    }
  }
